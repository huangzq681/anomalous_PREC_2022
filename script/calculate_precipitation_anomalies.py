...
The same research period of CMIP6 historical simulations are applied (1961-2014, total 54 years).
We applied an identical period from 2045-2098 (same length as the historical period) for the ssp585 scenario.

The processing procedures include:
1. Aggregate the CMIP6 simulations with the periods of history (1961-2014) and future (2045-2098) scenarios for each model respectively;
...
4. Calculate the multi-year means for each calendar day during 1961-1990 as the climatologies of each model;
...
6. To generate comparable results, the climatologies for the ssp585 scenario are calculated as the difference between the ssp585 time series 
and their climatological means from the historical simulations.
'''
            ...
            if forcing == 'historical':
                ...
                for i in range(2045,2099):
                    if os.path.exists(to_dir + forcing+'_pr_'+model+'_'+str(i)+'_masked.nc'):
                        ...
                        da = da * 86400
                        da.to_netcdf(to_dir + forcing+'_pr_'+model+'_'+str(i)+'_masked.nc')
            ds = xr.open_mfdataset(to_dir+'{}_pr_{}_*_masked.nc'.format(forcing,model), combine='nested', concat_dim='time', parallel=True)
            if forcing == 'historical':
                ds = ds['precipitation']
                prec_1961_1990 = ds.sel(time = slice('1961','1990'))
                ...
                prec_ano.to_netcdf(to_dir + forcing + '_pr_' + model + '_1961_2015_summer_anomalies.nc')
                prec_clim.to_netcdf(to_dir + forcing + '_pr_' + model + '_1961_1990_summer_climatologies.nc')
            else: # forcing == 'ssp585'
                ds = ds['precipitation']
                prec_clim = xr.open_dataarray(to_dir + 'historical' + '_pr_' + model + '_1961_1990_summer_climatologies.nc')
                prec_ano = ds.groupby('time.dayofyear') - prec_clim
                prec_array = prec_ano.values
                prec_array = (prec_array[:-4,:,:] + prec_array[1:-3,:,:] + prec_array[2:-2,:,:] + prec_array[3:-1,:,:] + prec_array[4:,:,:]) / 5
                prec_ano[2:-2,:,:] = prec_array
                prec_ano = prec_ano.sel(time = np.in1d(prec_ano['time.month'], [5,6,7,8,9,10]))
                prec_ano.to_netcdf(to_dir + forcing + '_pr_' + model + '_2045_2098_summer_anomalies.nc')
