...
We first concatenate the historical and ssp585 period, and then calculate the anomalies based on a running climatological mean.
...
for model in models:
    if model != 'GFDL-CM4_gr2':
        ...
        for i in range(1981,2100):
            prec_pre_20year = ds.sel(time = slice(str(i-20),str(i-1)))
            prec_clim = prec_pre_20year.groupby('time.dayofyear').mean()
            prec_ano_i = ds.sel(time = str(i)).groupby('time.dayofyear') - prec_clim
            if i == 1981:
                prec_ano = prec_ano_i
            else:
        ...
        prec_ano.to_netcdf(to_dir + forcing + '_pr_' + model + '_1981_2099_running_anomalies.nc')
